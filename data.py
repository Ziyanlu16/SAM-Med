...
def custom_collate_fn(batch):
    ...
    if len(batch) == 0:  # 如果批次中所有样本都是 None，则返回一个空批次
        return None
...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=256, mode='train', requires_name=True, point_num=1, mask_num=5, subset_fraction=1):
        """
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            with open('error_log.txt', 'a') as file:
                file.write(f"Error processing data at index {index}, file: {image_file_path}: {e}\n")
...
if __name__ == "__main__":
    ...
    train_batch_sampler = DataLoader(dataset=train_dataset, batch_size=1, shuffle=True, num_workers=4, collate_fn=custom_collate_fn)
