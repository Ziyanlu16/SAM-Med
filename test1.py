...
# # from segment_any.modeling.image_encoder import ImageEncoderViT
...
# # x = torch.randn([1,3,256,256]).float().to('cuda')
# # model = ImageEncoderViM(img_size=256).to('cuda')
# # x = model(x)
...
with open('/mnt/dataset/SAMeD/image2label_train2.json', 'r') as f:
    data = json.load(f)

with open('/mnt/SAM/trimmed_paths.txt', 'r') as f:
    paths_to_remove = [line.strip() for line in f.readlines()]

for image_path in paths_to_remove:
    data.pop(image_path, None)

# 将更新后的数据写回JSON文件
with open('/mnt/dataset/SAMeD/updated_image2label_train2.json', 'w') as f:
...
def predict_interface(image, args):
    ...
    if point is not None:
        x, y = point
        point_coords = [[x, y]]  
        transformed_point_coords = transform_point_coords(point_coords, original_size=(image.width, image.height), target_size=(args.image_size, args.image_size))
        point_coords = transformed_point_coords[0]  
    else:
        point_coords = None
