...
def validate(args, model, val_loader, criterion, local_rank):
    ...
    val_loss_tensor = torch.tensor(val_losses).cuda()
    dist.reduce(val_loss_tensor, 0)
    if local_rank == 0:
    ...
    for i in range(len(val_iter_metrics)):
        val_metric_tensor = torch.tensor(val_iter_metrics[i]).cuda()
        dist.reduce(val_metric_tensor, 0)
        if local_rank == 0:
            val_iter_metrics[i] = val_metric_tensor.item() / dist.get_world_size()
...
def train_one_epoch(args, model, optimizer, train_loader, epoch, criterion, scaler, local_rank):
    ...
    for batch, batched_input in enumerate(train_loader):
        if any(data is None for data in batched_input):
            if local_rank == 0:
        ...
        try:
            ...
            # if local_rank == 0:
            #     print(f"Epoch: {epoch+1}, Batch: {batch+1}, Loss: {train_losses[-1]}, Metrics: {train_iter_metrics}")
...
def main(local_rank, args):
    ...
    dataset = TrainingDataset(args.data_path, image_size=args.image_size, mode='train', point_num=1, mask_num=args.mask_num, requires_name=False)
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True, sampler=train_sampler, generator=g)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True, sampler=val_sampler, generator=g)
    ...
    for epoch in range(0, args.epochs):
        ...
        val_loss, val_metrics = validate(args, model, val_loader, criterion, local_rank)
        if local_rank == 0:
            loggers.info(f"Validation loss: {val_loss:.4f}, metrics: {val_metrics}")
